...
const UserLogin = () => {
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      <div className="bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 rounded-xl h-44 w-96">
    
        <h2 className="text-lg font-semibold mb-4">Welcome back</h2>
        <form className="w-full" onSubmit={(e) => e.preventDefault()}>
          <label className="block text-sm text-gray-500 dark:text-gray-400 mb-2">
            Email
            <input className="sh-input mt-1 block w-full h-11 px-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-transparent text-gray-900 dark:text-gray-100" type="email" placeholder="you@example.com" />
          </label>
          <label className="block text-sm text-gray-500 dark:text-gray-400 mb-2">
            Password
        ...
        <p className="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">Don’t have an account? <a href="/user/register" style={{ color: 'var(--accent)' }}>Create one</a></p>
      </div>
      </div>
  )
}
